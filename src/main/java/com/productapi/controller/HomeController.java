...
// http://localhost:9000/api/v1/ is the base URL
    ...
    public String token(Authentication authentication){
        System.out.println("Toekn requested for user: "+ authentication.getName() );
    ...
    public ResponseEntity<ProductDto> getProductById(@PathVariable Long id){
        System.out.println("@Controller:: Get product for ID: " + id) ;
        ProductDto productDto = null;
        try {
            productDto = productService.getProductById(id);
        } catch(Exception e){
           System.out.println("@Controller:: Exception occurred: " + e.getMessage());
            System.out.println("@Controller:: Exception occurred:productDto:: " + productDto);
